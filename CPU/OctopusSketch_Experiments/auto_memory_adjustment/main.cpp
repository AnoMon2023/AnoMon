...
interval_distribution(const vector<pair<TUPLES, uint32_t>> &interval_data, 
    ...
    octopus.memory_init(proportion);
    ...
    vector<int> light_packet_num = octopus.get_light_packet_num();
    vector<int> conflict_num = octopus.get_conflict_num();
    vector<double> used_counter_ratio = octopus.get_used_counter_ratio();
    vector<int> light_memory = octopus.get_light_memory();
